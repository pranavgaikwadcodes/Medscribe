...
if(isset($_GET['cancel']))
  ...
  if(isset($_GET["generate_bill"])){
    ...
    if(isset($_GET['ID']))
    {
      ...
      $checkIfPaid = mysqli_query($con,"SELECT paymentStatus FROM appointmenttb WHERE ID = '".$_GET['ID']."'");
      $row = mysqli_fetch_array($checkIfPaid);
      ...
      if($status == 1) {
        ...
        $content .= '
            <br/>
            <h2 align ="center"> Medscribe</h2></br>
        ...
        $query=mysqli_query($con,"update appointmenttb set paymentStatus='1' where ID = '".$_GET['ID']."'");
        if($query)
        {
...
<head>
    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary fixed-top">
        <a class="navbar-brand" href="#"><img src="./images/Pic1.png" style="width:30px; height:30px;" alt="Logo">
            &nbsp Medscribe</a>
            ...
            <ul class="navbar-nav mr-auto">
                <li class="nav-item">
                    <a class="nav-link" href="logout1.php"
                        style="top: 10px;position: absolute;right: 10px; background-color: white;color: black;border-radius: 25ppx !important;"><i
        ...
        <div class="row">
            ...
            <div class="col-md-8" style="margin-top: 3%;">
                <div class="tab-content" id="nav-tabContent" style="width: 950px;">
                    ...
                    $query = "select pid,ID,fname,lname,gender,email,contact,appdate,apptime,userStatus,doctorStatus,paymentStatus from appointmenttb where doctor='$dname';";
                    ...
                    while ($row = mysqli_fetch_array($result)){
                      ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php 
                    if(($row['paymentStatus']==1) && ($row['userStatus']==1) && ($row['doctorStatus']==1))  
                    {
                      echo "<div style='color:green;'>Completed</div>";
                    }
                    if(($row['paymentStatus']==0) && ($row['userStatus']==1) && ($row['doctorStatus']==1))  
                    {
                      echo "<div style='color:blue;'>Active</div>";
                    }
                    if(($row['paymentStatus']==0) && ($row['userStatus']==0) && ($row['doctorStatus']==1))  
                    {
                      echo "<div style='color:brown;'>Cancelled by Patient</div>";
                    }

                    if(($row['paymentStatus']==0) && ($row['userStatus']==1) && ($row['doctorStatus']==0))  
                    {
                      echo "<div style='color:red;'>Cancelled by Doctor</div>";
                    }
                        ?></td>
                    ...
                    else if(($row['userStatus']==1) && ($row['doctorStatus']==1))  
                        { ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form method="get">
                                            ...
                                            $checkIfPaid = mysqli_query($con,"SELECT paymentStatus FROM appointmenttb WHERE ID = '".$row['ID']."'");
                                            $row = mysqli_fetch_array($checkIfPaid);
                                            $status = $row["paymentStatus"];

                                            if($status == 1) {
                                            ?>
                                            <a href="doctor-panel.php?ID=<?php echo $ID;?>">
                                                <input type="hidden" name="ID" value="<?php echo $ID;?>" />
                                                <input type="hidden" name="pid" value="<?php echo $pid;?>" />
                                                <input type="submit" name="generate_bill" class="btn btn-success" value="Download Prescription" />
                                            </a>
                                            ...
                                            <?php
                                              ...
                                              <a href="doctor-panel.php?ID=<?php echo $ID;?>">
                                                <input type="hidden" name="ID" value="<?php echo $ID;?>" />
                                                <input type="hidden" name="pid" value="<?php echo $pid;?>" />
                                                <input type="submit" onclick="alert('Bill Paid Successfully');" name="generate_bill" class="btn btn-success" value="Bill Paid & Download" />
                                              </a>  
                                              <?php
                                            }
                                            ?>
